...
public class MyHashMap<K, V> implements Map<K, V> {
    ...
    public MyHashMap(int dOA) {
        elements = 0;
        dimensionsOfArray = 0;
        container = (MyNode<K, V>[]) new MyNode[dOA];
    }
    ...
    public Set<Entry<K, V>> entrySet() {
        ...
        for (int i = 0; i < container.length; i++) {
            if (container[i] != null) {
                ...
                EntrySet<K, V> bufSet = new EntrySet<>();
                while (bufMyNode != null) {
                    bufSet.setKey(bufMyNode.key);
                    bufSet.setValue(bufMyNode.value);
                    entries.add(bufSet);
    ...
    private static class EntrySet<K, V> implements Map.Entry<K, V> {
